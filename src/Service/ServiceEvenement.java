...
public class ServiceEvenement implements IEvenement<Evenement> {
    ...
    public void delete(Evenement e) {
        try {
            ...
            checkStatement.setInt(1, e.getId());
    ...
    public void update(Evenement evenement) {
        ...
        if (evenement.getDate() == null) {
            System.out.println("La date de l'événement est obligatoire");
            return;
        }
        ...
        if (evenement.getId_users() == null || evenement.getId_users().isEmpty()) {
            System.out.println("La liste des participants ne peut pas être vide");
            return;
        }
        ...
        try (PreparedStatement stmt = connection.prepareStatement(query)) {
            ...
            stmt.setString(4, participantsString);
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    List<Integer> participants = Arrays.stream(rs.getString("id_users").split(","))
                            ...
                            .collect(Collectors.toList());
                    return new Evenement(id, nom, date, lieu, participants);
                }
            }
        } catch (SQLException ex) {
            ex.printStackTrace();
